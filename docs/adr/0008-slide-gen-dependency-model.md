...
The `slide-gen` plugin ships nine skills (`sg-research`, `sg-outline`, `sg-draft`, `sg-optimize`, `sg-validate-graphics`, `sg-generate-images`, `sg-build`, `sg-full-workflow`, `build-cfa-deck`) that are thin wrappers around an `sg` CLI. Every skill body invokes `sg <subcommand>` directly (`sg research`, `sg outline`, `sg build`, ...); none of the actual research, drafting, optimization, or PowerPoint-assembly logic lives inside `plugins/slide-gen/`. The `sg` engine itself is a separate project at `github.com/davistroy/slide-generator`, installed independently via `pip install -e ".[all]"`.
