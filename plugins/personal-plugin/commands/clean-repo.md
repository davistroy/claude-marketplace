...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(git:*), Bash(grep:*)
---
...
**JSON Output Schema:** See `references/clean-repo-examples.md` for the full schema.
...
See `references/clean-repo-examples.md` for full example transcripts covering `--dry-run` mode, a default run, and `--docs-only --audit` mode.
