...
Generate the document using the full template in `references/recommendations-template.md`. Skeleton (section names only — see the reference for field-level structure):

```markdown
# Improvement Recommendations
**Generated:** [...]  **Analyzed Project:** [...]
## Executive Summary
## Recommendation Categories   — 3-7 categories from findings; prefix table U/Q/A/D/N/S/P/CI/DH; each item has Priority/Effort/Impact/Current State/Recommendation/Implementation Notes
## Quick Wins                 — Impact/Effort matrix: Critical/High priority + XS/S/M effort
## Strategic Initiatives      — Impact/Effort matrix: Critical/High priority + L/XL effort
## Not Recommended            — minimum 3 items: Why Considered / Why Rejected / Conditions for Reconsideration
```

Emit the full document per the reference template — do not abbreviate categories, tables, or Not Recommended items when writing RECOMMENDATIONS.md.
...
**Before writing, check if IMPLEMENTATION_PLAN.md already exists.** If it does not exist, create it fresh with the structure below. If it exists, check whether every work item's `**Status:**` is `COMPLETE`: if so, offer to archive the completed plan to `docs/archive/` before starting fresh, or to append after it. If items are PENDING/IN_PROGRESS, append directly — locate the `<!-- BEGIN/END PHASES -->` and `<!-- BEGIN/END TABLES -->` markers, renumber new phases/items to continue from the highest existing number, and insert before `<!-- END PHASES -->` behind a separator comment.

Follow the complete procedure — including the archive branch, marker mechanics, and a before/after example — in `references/plan-append-guide.md`, using separator comment `<!-- Appended on [YYYY-MM-DD HH:MM:SS] from /plan-improvements -->`.
...
**Command-specific field values for `/plan-improvements`:**
- **Based On:** RECOMMENDATIONS.md
- **Ref field:** Use `Recommendation Ref:` with recommendation IDs (e.g., U1, A2)
- **Success Metrics:** Include impact metrics derived from the recommendations
- **Traceability:** Title the appendix "Recommendation Traceability" with columns: Recommendation, Source, Phase, Work Item (Source is always RECOMMENDATIONS.md)
- **Footer:** `*Source: /plan-improvements command*`
...
2. **Model Tier** — assign per the rubric in `references/plan-template.md` rule 17 (three tiers: haiku, sonnet, opus, ordered by capability need). Default to the lowest tier that plausibly works; for borderline items, choose the lower tier and add an escalation criterion to Notes.
...
**Complexity scale (S/M/L):** See `references/plan-template.md` → Sizing Constraints for phase/work-item size limits (files changed, LOC).
...
**Primary mechanism: per-item Model Tier** (Work Item Construction Guidelines step 2 above). Each work item already carries a `**Model Tier:**` field that `implement-plan` reads directly. The Execution Hints section provides **phase-level overrides** — use it only when an entire phase warrants a uniform departure from item-level tiers, or to set a context budget hint.
...
If `AGENTS.md` does not exist in the repo root, offer to generate one from CLAUDE.md and the codebase analysis (skip entirely if it already exists — never overwrite). On acceptance, extract project context from CLAUDE.md and emit `AGENTS.md` per the template and generation rules in `references/agents-md-template.md` (tool-agnostic — no Claude Code-specific features).
...
(2 Critical, 6 High, 10 Medium, 5 Low).
...
Files saved: RECOMMENDATIONS.md, IMPLEMENTATION_PLAN.md
```

Run with `--recommendations-only` (alias `--no-plan`) to stop after RECOMMENDATIONS.md is saved — the report then omits plan details and points to `/create-plan RECOMMENDATIONS.md` to generate the plan later.
