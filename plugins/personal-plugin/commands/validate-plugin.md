...
**Validation:** If required arguments are missing, display the usage message: the full flag list, the eight example invocations, and the dynamically-discovered plugin list. If `plugin-name` is not found (and `--all` not specified), display an error naming the missing plugin plus the same discovered plugin list and a hint to use `--all`. See `references/validate-plugin-examples.md` for the exact text of both messages.
...
Check for `plugin.json` (required); at least one of `commands/` (flat `*.md` files, filename becomes command name) or `skills/` (nested `[skill-name]/SKILL.md` directories, uppercase filename required). See `references/validate-plugin-examples.md` for the required directory structure diagram.
...
**Check for each item in skills/ directory:** it is a directory (not a file), and it contains `SKILL.md` (exact name, uppercase).

Output samples: see `references/validation-output-examples.md` §1.2

**Auto-fix with --fix:** automatically restructure invalid skills (flatten files into `skills/name/SKILL.md`, fix lowercase filenames). Sample confirmation: see `references/validation-output-examples.md` §1.2

#### 1.3 plugin.json Validation

**Check:** file is valid JSON (parseable); required fields `name`, `description`, `version` are present; `version` follows semver format (X.Y.Z).
...
**Valid Plugin Entry Fields:** `name`, `source`, `description`, `version` (all required); `category`, `tags` (optional).

**Known Invalid Fields:** `last_updated` - not part of Claude Code's plugin schema.

**Check:** parse `.claude-plugin/marketplace.json`; for each plugin entry, flag any field not in the valid fields list above.

Output samples: see `references/validation-output-examples.md` §1.4

**Auto-fix with --fix:** automatically remove unrecognized fields. Sample confirmation: see `references/validation-output-examples.md` §1.4
...
For each markdown file: check for frontmatter delimiters (`---` at start), parse the YAML between delimiters, and report any syntax errors.
...
**For Commands** (files in `commands/`): check that no `name` field is present.

**For Skills** (files in `skills/*/SKILL.md`): check that `name` field IS present and matches the directory name.

Output samples: see `references/validation-output-examples.md` §2.3

#### 2.4 Optional Field Validation

If `allowed-tools` is present, check it's a valid string format and warn if the format appears incorrect (e.g., missing parentheses).
...
**Check these files:** `plugins/[plugin-name]/.claude-plugin/plugin.json` -> `version` field; `.claude-plugin/marketplace.json` -> plugin entry's `version` field.
...
Verify markdown parses without errors: check for unclosed code blocks, malformed links, and unbalanced formatting.

Output samples: see `references/validation-output-examples.md` §4.1

#### 4.2 Code Block Language Specifiers

Check that fenced code blocks have language specifiers. Good/bad code-fence examples and report/warning samples: see `references/validation-output-examples.md` §4.2

#### 4.3 Internal Link Validation

Check that internal file references exist, including patterns like "See common-patterns.md", `[link](../references/file.md)`, and references to other commands.
...
For each plugin, build a registry of command and skill names, dynamically discovered from `commands/*.md` and `skills/*/SKILL.md`. See `references/validate-plugin-examples.md` for an example namespace registry.
...
When validating a single plugin (not `--all`), skip collision detection and display a note explaining why.
...
If a `dependencies` field exists in plugin.json, parse it. Example `dependencies` block and validation samples: see `references/validation-output-examples.md` §6.1

#### 6.2 Validate Dependencies

For each declared dependency: check if the plugin exists in the marketplace, parse the version requirement (semver syntax), and compare against the installed plugin version.

Output samples: see `references/validation-output-examples.md` §6.2

#### 6.3 Semver Validation

Check that version strings follow semver patterns: `>=X.Y.Z`, `<=X.Y.Z`, `>X.Y.Z`, `<X.Y.Z` (comparison); `^X.Y.Z` (caret range - compatible with); `~X.Y.Z` (tilde range - approximately); `X.Y.Z` (exact version).
...
**Check for hooks in these locations:** `plugins/[plugin-name]/hooks/hooks.json` (in marketplace); `%USERPROFILE%\.claude\plugins\cache\*/[plugin-name]/*/hooks/hooks.json` (installed).
...
**Bash Script Indicators:** command starts with `bash ` or `sh `; contains `/bin/bash` or `/bin/sh`; references a `.sh` file extension; uses `${CLAUDE_PLUGIN_ROOT}/hooks/*.sh`.

Output samples: see `references/validation-output-examples.md` §7.2

#### 7.3 Check for PowerShell Equivalents

For each bash script found, check if a PowerShell equivalent exists: if `hooks/stop-hook.sh` exists, check for `hooks/stop-hook.ps1`; verify hooks.json has a Windows-compatible alternative configured.

Output samples: see `references/validation-output-examples.md` §7.3

#### 7.4 Windows Compatibility Summary

Emit one of three summaries depending on detection: **bash-only hooks detected** - warn, listing the bash scripts without PowerShell equivalents and how to fix them; **all hooks Windows-compatible** - report PASS; **no hooks exist** - report PASS (no hooks configured).

Output samples: see `references/validation-output-examples.md` §7.4

#### 7.5 Hook Script Syntax Validation

For any bash scripts found, perform basic syntax validation: shebang line present (`#!/bin/bash` or `#!/usr/bin/env bash`); no Windows-incompatible hardcoded paths (`/home/`, `/usr/`, etc.); no missing closing brackets/braces.
...
For each command markdown file, validate frontmatter against these rules: `description` field present (required) and between 10-200 characters; no forbidden `name` field present; `allowed-tools` format valid if present.

Output samples: see `references/validation-output-examples.md` §8.1

#### 8.2 Required Sections Check

Verify each command contains the required sections: `## Input Validation` (must document arguments) and `## Instructions` (must have step-by-step guidance).

Output samples: see `references/validation-output-examples.md` §8.2

#### 8.3 Output Naming Convention Compliance

Check that commands generating output follow the naming pattern `[type]-[source]-[timestamp].[ext]`, looking for output file naming in documentation and examples showing correct naming.

Output samples: see `references/validation-output-examples.md` §8.3

#### 8.4 Error Message Format Adherence

Check that commands document error handling following the standard format defined in `references/common-patterns.md` under "Error Message Format" (`Error:` / `Expected:` / `Received:` / `Suggestion:`).
...
**Check:** `plugins/[plugin-name]/references/plan-template.md` exists. If the file is missing, skip the rest of Phase 8.5.
...
**Check:** at least 17 structural rules are defined (numbered 1-17); no gaps in numbering (1, 2, ... N with no missing numbers).
...
**Check for each rule:** the numbered rule exists in the structural rules list, and the rule text contains all required keywords (case-insensitive).
...
**Check:** section `## Sizing Constraints` exists; contains "Maximum phases per plan" with a numeric limit; contains "Maximum work items per phase" with a numeric limit.
...
**Top-level files (`references/`):** `plan-template.md`, `common-patterns.md`, `anti-patterns.md`, `adr-template.md`, `agents-md-template.md`, `flag-consistency.md`, `validation-maturity-scorecard.md`, `plan-append-guide.md`, `recommendations-template.md`, `create-plan-examples.md`, `implement-plan-state-schema.md`, `validation-output-examples.md`.

**Hook reference files (`references/hooks/`):** `planning-stop-hook.md`, `verification-post-edit-hook.md`, `session-start-hook.md`.

**Required non-empty subdirectories:** `references/patterns/` (at least one file); `references/templates/` (at least one file).
...
Read each discovered `plugin.json` to extract the `version` field. Also read the local marketplace registry: `.claude-plugin/marketplace.json` in the repository root.
...
If the `gh` command fails (not installed, not authenticated, no network), fall back to a local-only comparison (Step 9.5.3 compares local files against each other only). See `references/validate-plugin-examples.md` for the exact fallback message and `gh` setup instructions.

#### 9.5.3 Compare Versions

For each locally discovered plugin:

**From remote marketplace.json (if available):** latest version available on the remote repository.

**From local plugin.json:** currently installed version.

**From local marketplace.json:** locally registered version.
...
- Categorize update type: **MAJOR** — Breaking changes (X.0.0); **MINOR** — New features (0.X.0); **PATCH** — Bug fixes (0.0.X)
...
Report the exit code at the end (e.g. `Validation complete. Exit code: 0 (success)` or `Validation complete. Exit code: 1 (errors found)`).
