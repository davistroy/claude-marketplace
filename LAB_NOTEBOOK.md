...
| Item | State |
|---|---|
| Versions | marketplace **3.3.0** - personal-plugin **11.9.0** (E066) - bpmn-plugin **4.4.0** - slide-gen **1.4.0**. 11.7.0 -> 11.8.0 (#235 plan-identity fingerprint) -> 11.9.0 (#232 mitigations). **Caveat:** bpmn-plugin's installed 4.4.0 differs in CONTENT from the repo's 4.4.0 -- its CHANGELOG is 194 lines against 214 -- because `claude plugin update` compares version strings and CHANGELOG.md is bump-exempt. Exempt-from-bump also means never-reaches-an-installed-user (A25). |
...
| Open backlog | **8 open**, no P0 or P1. #198/#199/#200/#216/#238 are the scope of plan v14; #169/#170/#171 are v2 features deferred by design (each says `Priority: low / someday` in its own body). #206 closed on its inventory half only; its context-economy half was refiled as **#238** (A24). **Trap that recurs:** a comma-separated `Closes #A, #B, #C` in a PR body closes only its leading entries -- it left 10 issues open on PR #236 and each had to be verified against merged `main` and closed by hand (E064). |
