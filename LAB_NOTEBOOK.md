...
|---|--------|---------|-------------|
| A24 | **Refile #206's context-economy half.** #206 was closed by PR #236 on its inventory half (the `update-readme.py` generalization); its second half -- oversized SKILL bodies against the <500-line budget -- shipped nothing and is now tracked nowhere. File it as its own issue before it is forgotten. | 2026-07-30 | E064 |
...
| Open backlog | **9 open** after PR #236 (E064) took it 21 -> 9; 12 closed, 10 of them by hand because a comma-separated `Closes` list only closes its leading entries. Remaining, grouped by root cause: **(4) calibration, DEFERRED** #198/#200/#199 -- the 13/14 eval gives no evidence these dials are mis-set - **(5) label-vs-content** #232 (loader served 11.3.0 while the manifest named 11.6.0), #235 (state file names its plan by path) - **(6) own plan each** #216, #169/#170/#171. **No P0 or P1 remains open.** Note: #206 closed on its inventory half; its context-economy half (oversized SKILL bodies) is now **untracked** and needs refiling. |
