...
| — | The canonical backlog is the GitHub issues list (A2/A12), now **9 open**. **Traps that recur:** a comma-separated `Closes #A, #B, #C` in a PR body closes only its leading entries -- write the keyword before each number, or verify the close set after every merge (E064); `gh issue view <n>` silently resolves PR numbers; an artifact identified by a *label* rather than its *content* has now bitten four times (#226 version string, #232 cache path, #235 plan path, and bpmn-plugin's 4.4.0 cache in E064) -- verify content, never the identifier; `bws secret list` prints plaintext; any injection linter must **replay the pre-pass, not grep**; and a Definition-of-Done row must be **derived from the gate it mirrors**. | — | E043 |

### Completed

| # | Action | Created | Completed | Source Entry |
|---|--------|---------|-----------|-------------|
| C29 | **A26 CLOSED -- plan v14 merged (PR #247, squash `0f81cd5`) and the close set VERIFIED BY LISTING, not by trusting the merge.** All 24 checks green on both OS legs, including the two new ones. The five in-scope issues (#198/#199/#200/#216/#238) all closed at 16:33:45-47Z; open count went 14 -> **9**, matching the prediction exactly. Writing `Closes` before EACH number worked -- E064's comma-separated trap, which left ten issues open on PR #236, did not recur. Remaining 9 = 3 v2 deferrals (#169/#170/#171) + 6 new follow-ups (#241-#246). | 2026-07-31 | 2026-07-31 | E068 |
...
*Verified 2026-07-31 against `origin/main` @ `0f81cd5` -- plan v14 merged (PR #247, 25/25 items, 24/24 checks green).*
...
| Open backlog | **9 open**, no P0 or P1 -- verified by listing after the merge, not inferred from it (A26/C29). 3 are v2 features deferred by design (#169/#170/#171, each says `Priority: low / someday` in its own body); 6 are plan v14 follow-ups: **#241** (effort A/B -- v14 removed its confound), **#242** (tier-boundary replay), **#243** (`summarize-feedback` find-grant gap, same dispatch-time-only class as #216), **#244** (root CHANGELOG unguarded, same class as #210), **#245** (promote the non-required research-sse job to required), **#246** (`planning.md` mints a no-op `effort: high`). |
