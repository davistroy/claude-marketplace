...
**Status:** IN PROGRESS — all 5 items implemented + local gates green; PR pending. This PR is the highest-risk round (it rewrites the gate workflows themselves — the mypy-ratchet shell, pip-audit `--requirement`, and SHA-pinned actions can only be fully confirmed by CI). Entry closed with the squash-merge SHA at the start of the Phase 5 branch.
