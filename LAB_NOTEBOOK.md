...
| D15 | One escalation per item allowed (lower → next tier); accept at the highest tier even if imperfect | 2026-05-10 | ACTIVE | E005 (see also D72) | Unlimited escalation loop — rejected because it can cycle; capping at one step keeps orchestrator budget bounded |
| D16 | Orchestrator advisory note in `implement-plan.md` recommends Opus for large plans; not enforced programmatically | 2026-05-10 | ACTIVE | E005 (see also D72) | Skip the note — rejected because a cheap orchestrator with wrong tier assignment costs more than its token savings |
...
| D53 | **A bundled skill that needs a Bitwarden secret must `eval "$(grep -m1 '^export BWS_ACCESS_TOKEN=' ~/.config/claude-env.sh)"` before calling `bws`, never trust the inherited env var** — ~~`~/.bashrc:167` loads that file behind `case $- in *i*)`, so a non-interactive tool shell keeps whatever stale token it inherited~~ **Correction (E063):** `~/.bashrc:167` guards the *full credential set*, not this mechanism specifically; the operative line is `~/.bashrc:8`, an early `return` for non-interactive shells that makes the anti-staleness `eval` at `:165` (present since 2026-07-16 — already #217's proposed fix) unreachable. The E058 stale-token incident was real but transient and host-level; it does not reproduce | 2026-07-29 | ACTIVE | E058 | Treat `400 invalid_client` as a revoked token and rotate — rejected, the token was valid; the shell was loading a retired machine account (`da8557b9…`) while the file held the current one (`f283b1a6…`). Treat it as a region mismatch — rejected by measurement, US *and* EU identity endpoints both rejected the stale token |
...
| D60 | **`tasks.json`'s git status is an OPEN design question (#230), not a settled one** — `.gitignore` ignores it while D34 and `docs/plans/2026-07-18-task-sync-design.md:64,100,129` all specify it committed "so the base travels between the user's two machines" | 2026-07-30 | **RESOLVED by D65** (2026-07-30) | E062 | Fix inline by un-ignoring it — rejected: PR #175's public-repo reasoning is defensible even though the confidentiality subsystem already answers it, so this is a design call needing an explicit decision + alternatives, which is precisely what its absence cost the first time. Leave it undocumented — rejected: three sources currently disagree and each reads as authoritative alone |
...
| D66 | **Plan v14's scope is set by INVESTIGATED shape, not filed text -- and three filed deliverables were reversed or dropped on evidence.** #216 ships the **transport rewrite only**, ladder unchanged; #238 ships as a **reframed authoring gate**, not a context-economy one; #198 ships **Rule 17 only**, with no replay experiment. | 2026-07-31 | ACTIVE | E067 | #216's filed ladder (`medium`/`high`/`xhigh`) -- rejected: current guidance for the default model says start mid-range and sweep *down*, calls the top tiers "for measured wins, not a starting point", and states the effort dial is not a reliable lever on visible output length, which is exactly what a research leg produces; shipping it would cost ~2x more for an arguably worse-aligned ladder. #238 as filed -- rejected: bodies are not always-loaded, so its cost model is wrong; kept anyway on a different argument (both over-budget files crossed in one commit whose plan budget-checked a *different* file). #198's replay experiment -- deferred: the one change the in-repo evidence supports (15 sonnet items at >=3 files, zero escalations, vs two opus items that were a one-line edit and a zero-file task) ships without it. |
| D67 | **A no-op edit that also removes a user control is a REGRESSION, not neutral -- so `effort: high` is never added.** An absent `effort:` frontmatter field resolves to exactly `high`, proven from the resolver, the model catalog, and the final coercion. | 2026-07-31 | ACTIVE | E067 | Apply #199's three `high` recommendations as filed -- rejected: all three are no-ops, and the issue calls one of them its "clearest case". Ship the whole issue as filed -- rejected: its priority ordering is inverted, since only the `low`/`medium` items move anything and they are *downgrades* it never labels as such. **Explicitly NOT claimed:** that writing the field removes the session override -- the investigator asserted it, but its own quoted precedence chain appears to contradict it, and the action is identical either way. |
| D68 | **Verification agents get verified.** Three subagent findings this session were wrong or unsourced and were corrected before reaching the plan: a "64K output ceiling" that is actually a **floor** (the ceiling is 128K); a removes-user-control claim contradicted by its own quoted code (see D67); and a tier-premium argument resting on ~1.7x when the premium is **2.5x today**, reaching 1.7x only after 2026-08-31 -- so no default was flipped. | 2026-07-31 | ACTIVE | E067 | Take the fan-out's findings as ground truth -- rejected on this repo's own record: E062 established that verification reduces but does not zero the error rate, and a second independent reader is what catches the residue. The pricing claim specifically was re-checked against the sanctioned `claude-api` source rather than inherited, per the standing rule. |
| D69 | **`/ultra-plan` gets an explicit `effort: xhigh` when the live keyword is deleted from its body.** Deleting the keyword alone is a **net de-escalation**, not a neutral cleanup -- the skill runs today at default `high` *plus* a live prompt-level injection, and its work (deep multi-file investigation with synthesis across a 6-agent fan-out) is the intelligence-sensitive profile. `xhigh` replaces an implicit, unsanctioned escalation with an explicit, sanctioned one. | 2026-07-31 | ACTIVE | E068 | Leave the field absent and accept the default `high` -- rejected, though it is the internally-consistent-looking option: D66 declined #216's ladder escalation on the guidance that top tiers are "for measured wins, not a starting point," and that argument transfers only if this were *raising* a component above its baseline. It is not -- it is preserving an escalation that exists today, which is a different question. Cost is bounded because the skill runs once per planning cycle. `effort: high` -- rejected as a **regression disguised as a no-op**: it changes nothing behaviourally (D67) while pinning a value the default already supplies. |
| D70 | **Plan v14's Phase 6 runs unattended rather than behind a review gate**, despite carrying the plan's one Critical risk (the refusal guard that relocates from a top-level field into a delta event under streaming). | 2026-07-31 | ACTIVE | E068 | Pause before Phase 6 for a design walkthrough -- rejected by the owner: the phase's own DoD already mandates fixture-first ordering and a mutation test per guard, so the gate a human review would apply is already encoded and runnable. Pause at every phase -- rejected as disproportionate for 8 phases whose other 7 are low-risk. **Note the residual:** if the mutation tests are written to pass rather than to fail-first, nothing catches it; that is the specific thing to check at Phase 6 acceptance. |
| D71 | **`CLAUDE.md`'s SKILL.md body-budget rule is re-framed as authoring quality, not context economy** — the harness loads a body only on invocation, so the always-loaded surface is the `description`, and the `<500`-line figure is an Anthropic authoring best-practice, not a platform limit. This is a REGRESSION in the repo's own understanding, not a new discovery: `LAB_NOTEBOOK.md:166` (E032) already recorded it as "a house rule, not a CI gate." | 2026-07-31 | ACTIVE | E068 | Delete the line budget outright -- rejected: Phase 7 still gates it, on the authoring-quality argument. Keep the context-economy rationale -- rejected as factually false. |
| D72 | **Three of #198's four prose claims are refuted, and are recorded here rather than silently dropped.** Claim 1c is wrong: `sonnet-implementer.md:31`'s escalation trigger, confirmed by reading the file, is already doubly qualified as multi-file refactoring *with system-wide coupling not anticipated in the plan* — not multi-file-ness per se — which is why item 5.1 left that file byte-identical. Claim 3 is wrong twice over: "no escalation above Opus" is D15 (cross-ref below, text/status untouched), ACTIVE since 2026-05-10 with a recorded rejected alternative, and it is literally true for this repo, which has no fable-tier implementer agent and no mechanism to dispatch one. Claim 4's premise — "the advisory is satisfied by default now" — is unverifiable in-repo, since nothing here sets a session model, and the advisory it proposes to reword is D16 (cross-ref below, text/status untouched), whose deletion was already considered and rejected. Standing tier-calibration evidence, recorded with its denominator: across all archived plans, exactly **one escalation in 162 tiered items** — item 4.2's sonnet-to-opus step — attributable to a self-contradictory spec (the ≤500-line target conflicted with the byte-intact clause) rather than to model capability. | 2026-07-31 | ACTIVE | E068 | Silently drop the three refuted claims as filed -- rejected: a future reader who finds them undone with no explanation will simply re-derive them. Edit D15/D16's own text or status to reflect #198's framing -- rejected: both decisions remain ACTIVE and correct as originally written; #198 was wrong about them, not the reverse. |
...
|---|--------|---------|-------------|
| A26 | **Verify the close set after plan v14's PR merges, by LISTING -- do not trust the merge.** The PR body writes `Closes` before each of #198/#199/#200/#216/#238 (a comma-separated list closes only its leading entries, E064). Expected open count after merge: **9**. If any of the five remain open, close by hand and re-verify. | 2026-07-31 | E068 |
| A25 | **Decide whether the bump-exemption's distribution consequence needs stating.** `EXEMPT_EXACT`/`EXEMPT_GLOBS` in `check_version_bump.py` correctly exempt CHANGELOG/LICENSE/README/tests/examples from a version bump -- and because propagation is keyed on the version string, those paths therefore **never reach an installed cache**. Demonstrated live: bpmn-plugin 4.4.0 is installed with a 194-line CHANGELOG against the repo's 214 and `claude plugin update` says "already at the latest version". Harmless for four of the five classes; `examples/**` is user-facing. | 2026-07-30 | E064 |
| — | The canonical backlog is the GitHub issues list (A2/A12), now **14 open** (9 once plan v14's PR merges and closes #198/#199/#200/#216/#238). **Traps that recur:** a comma-separated `Closes #A, #B, #C` in a PR body closes only its leading entries -- write the keyword before each number, or verify the close set after every merge (E064); `gh issue view <n>` silently resolves PR numbers; an artifact identified by a *label* rather than its *content* has now bitten four times (#226 version string, #232 cache path, #235 plan path, and bpmn-plugin's 4.4.0 cache in E064) -- verify content, never the identifier; `bws secret list` prints plaintext; any injection linter must **replay the pre-pass, not grep**; and a Definition-of-Done row must be **derived from the gate it mirrors**. | — | E043 |

### Completed

| # | Action | Created | Completed | Source Entry |
|---|--------|---------|-----------|-------------|
| C28 | **A24 CLOSED -- #206's context-economy half refiled as #238.** Re-measured rather than copied forward, which caught three drifts: `lab-notebook` had grown 499 -> **540** (over budget, unreported), `visual-explainer` at **523** was never in #206's list, and `release-plugin`'s box-drawn templates were **already gone**. Root cause named in the new issue: the budget is stated in `CLAUDE.md` and enforced nowhere | 2026-07-30 | 2026-07-30 | E064 |
| C27 | **A22 CLOSED — `security-analysis`'s prose gate converted to `AskUserQuestion`** (#223), together with `wiki:207`/`:241` and `task-sync:247` per D64, plus the three `references/` generator templates that were still minting `(y/n)` prompts. The frontmatter hazard was navigated (insert before the ` #`; the trailing comment contains a colon-space) and verified by `yaml.safe_load`, not just `--strict`. Caught in passing: a 13-char `AskUserQuestion` header, which exceeds the 12-char cap and would have failed at runtime | 2026-07-29 | 2026-07-30 | E063 |
...
*Verified 2026-07-31 against `feature/plan-v14` @ `0fa09af` (plan v14, 25/25 items; PR pending merge to `main`).*

| Item | State |
|---|---|
| Versions | marketplace **3.3.0** - personal-plugin **11.10.0** (E068) - bpmn-plugin **4.4.0** - slide-gen **1.5.0**. 11.9.0 -> 11.10.0 and 1.4.0 -> 1.5.0 both from plan v14; bpmn-plugin deliberately NOT bumped (0 files changed, D45). **Caveat unchanged:** bpmn-plugin's installed 4.4.0 differs in CONTENT from the repo's 4.4.0 -- CHANGELOG.md is bump-exempt, and exempt-from-bump also means never-reaches-an-installed-user (A25). |
...
| Bundled tools | visual-explainer **913 tests / 93.38%** - bpmn2drawio 642 / 92.79% - feedback-docx 69 / 96.95% - task-sync 480 / 96.30% - **research-sse 87 / 99.06% (NEW, E068)**. All bare-`mypy` clean (D33) and ruff clean. `research-sse` is stdlib-only by design -- it parses a stream curl already fetched -- which is what makes its whole suite runnable offline with no key. Its CI job is **NON-required** for now (#245). |
...
| CI | `test.yml` (pytest x2 OS per tool, coverage floors, pip-audit per lockfile, advisory `Python Compat (3.10)/(3.12)`, **new non-required `Research SSE Tests` job**) - `validate.yml` (plugin/frontmatter/version-sync, README-sync `--check`, eval linter, injection linter, ruff, markdownlint, **new `Check SKILL.md body budget` STEP** in the existing `plugin-validate` job -- a step, never a new job, so no required-check name changed). **Branch protection on `main`: 16 required checks**, PR-required 0-approvals, `enforce_admins=false` (ADR-0007/D22). |
...
| Open backlog | **14 open** pre-merge, **9 after** the PR merges (#198/#199/#200/#216/#238 close with it). No P0 or P1. #169/#170/#171 are v2 features deferred by design. Six NEW follow-ups from plan v14: **#241** (effort A/B -- its confound was only removed by v14), **#242** (tier-boundary replay), **#243** (`summarize-feedback` find-grant gap, same class as #216), **#244** (root CHANGELOG unguarded), **#245** (promote research-sse to required), **#246** (`planning.md` mints a no-op `effort: high`). **Trap that recurs:** a comma-separated `Closes #A, #B, #C` closes only its leading entries -- the PR body writes the keyword before each number, and the close set must still be verified by listing after merge (E064). |
...
- **#217** — ~~`/unlock` and the `~/.claude/scripts/*` helpers should source `~/.config/claude-env.sh` rather than trust the inherited `BWS_ACCESS_TOKEN` (D53); the stale-inherited-token failure recurs in every non-interactive tool shell.~~ **Correction (E063):** the real in-repo defect was `/unlock` reading the unset `$TROY` instead of `BWS_ACCESS_TOKEN` (fixed in Phase 5); the `.bashrc:167` mechanism was misattributed — see corrected D53 (`.bashrc:8` is operative). The E058 stale-token incident was transient and host-level and does not reproduce. Also covers the `bws secret list` plaintext-leak hygiene fix.
...
| 2 | 2.1-2.4 | opus | e13e83a + (this) | self-test ✔ required-checks=16 ✔ pre-commit ✔ stdlib-only ✔ job-names identical ✔ | Release-integrity gate live: CI step + pre-commit Check 7 |

**Phase 2's gate is two conditional rules, not one, and the tension is the reason.** Rule 1 requires a version bump when bump-worthy paths under `plugins/<name>/` change; Rule 2 requires a CHANGELOG entry when the version changes. **`plugins/*/CHANGELOG.md` is EXEMPT from Rule 1 and MANDATORY under Rule 2** — item 1.1 was a CHANGELOG-only change with no bump, and a single-rule gate would have rejected the very PR that closed #210. Rule 2 asserts entry *presence* only and deliberately never date parity: seven versions carry dates that disagree between the root and per-plugin CHANGELOGs, all pre-existing on `main`, so a parity assertion would redden the gate on legacy history the day it landed — the D55 hazard exactly.

**Negative-tested before wiring, with observed exit codes:** skill-edit-no-bump **1**, bump-without-changelog **1**, bump-with-changelog **0**, changelog-only-no-bump **0** (that last run against `b4d576c...f4d07b6`, which *is* Phase 1's own commit — real history rather than a fixture). `--self-test` carries 18 cases of which **10 assert exit 1**, and was mutation-tested with 8 mutants; one initially **survived**, exposing a substring-collision fixture that was not actually a collision, which was then replaced with two real ones.

**A false-positive diagnostic nearly became a real defect.** Pyright flagged `changelog_has_entry(text, new_version)` as `str | None`, which read as a crash path on plugin deletion. I wrote a guard for it. The negative test then showed the case passed *with the guard removed* — because the absent-manifest guard four lines earlier already handled it. My "fix" was dead code and my test comment asserted a crash that never existed. Deleting the **real** guard makes the self-test crash with `TypeError: ... NoneType found` and exit 1, so the case is now pinned to something that can actually fail. Replaced the branch with a type-narrowing `assert` carrying a comment explaining why it must stay an assert: a runtime branch there looks like a handled case and invites someone to delete the guard above it.

**Process failure, mine, worth the entry.** The first negative-test run created a scratch branch *mid-phase* while 2.1's and 2.2's work was uncommitted; `git add -A` swept it into the scratch commits and deleting those branches removed `check_version_bump.py` and the `fetch-depth` edit from the tree. Recovered in full from a scratchpad copy plus `git reflog` (`c7cc067`). **Root cause: I skipped the per-batch commit.** `/implement-plan` commits at every batch boundary and 2.1/2.2/2.3 are separate batches — that commit is not bookkeeping, it is the property that makes scratch-branch work and `git checkout -- .` rollback safe. The lesson generalizes past this command: never run a branch-switching experiment with uncommitted work in the tree.

**Two self-corrections caught by the next reader rather than by me.** The 2.4 agent found my 2.3 note claiming "19 self-test cases" when the script prints 18 — I had counted with `grep -c PASS`, which also matched the trailing `self-test PASSED` line. And the pre-commit filter it added deliberately delegates bump-worthy classification to the script's own `EXEMPT_EXACT`/`EXEMPT_GLOBS` rather than restating it in shell, so the local check cannot drift into disagreeing with the CI one — the derive-don't-copy rule applied to a shell wrapper.
| 3 | 3.1-3.2 (3.3 descoped) | sonnet | 5743d61, f8e995e | eval-linter ✔ mdlint ✔ | Harness documented; 6 fragile scenarios re-scoped |
| 4 | 4.1-4.2 | sonnet | (this) | no-L-taxonomy ✔ :350-intact ✔ budget 385/500 ✔ validate ✔ evals ✔ version-gate ✔ | 29 numbering sites + eval:43 + adr-template; personal-plugin 11.7.0 |

**Phase 3's real scope was 6 scenarios, not the 1 filed — and one of the six was a false green.** S7's criterion is structurally identical to S4's failing one and passed only because `superpowers:brainstorming` happened not to fire that run, while being cited in both #227 and E061 as *proof the handoff works*. Fixing S4 alone would have left the same defect green in five other scenarios and kept the false evidence live. The S8 template is what made the fix mechanical: name no skill you must dispatch to, assert *recognition* rather than dispatch, hedge the behavioural clause.

**The eval linter was negative-tested against this specific file before the green was accepted.** Stripping S4's `Must` blocks exits 1 naming the scenario; renaming `## Rubric` exits 1. An earlier attempt at a negative test was itself defective — mangling `### S4:` to `### S4-broken:` did *not* fail, because `-` is a word boundary and `^S\d+\b` still matched. Worth recording: a negative test can be wrong in the direction of *passing*, which looks exactly like the thing it was supposed to prove.

**Phase 4's trap was real and would have inverted the fix.** `evals/skills/ultra-plan.eval.md:43` had copied the phase-numbering defect verbatim from `SKILL.md:167`, so repairing the skill alone would have turned a passing eval into a false failure against a now-correct skill — the standing "a check that restates an external truth drifts into agreeing with the bug" rule, with the eval as the check. Both moved in one commit. The `:350` do-not-touch held: the `:349-356` mapping table is byte-identical to `main`, and `:363`/`:385` — which had named the same artifact as "Phase 5 summary" and "Phase 4 summary" respectively — now agree.

**The phantom taxonomy was load-bearing, not cosmetic.** It gated three behaviours (constitution check, ADR emission, creative branching) on a value `plan-gate` cannot produce — its output is a Path recommendation, not a level. Deleted at all 10 sites including both `references/adr-template.md` sites, which is the file `SKILL.md:229` loads at runtime: fixing only the skill body would have left the template minting the phantom into every ADR generated from it.
| 5 | 5.1-5.2 | sonnet | (this) | no-bare-probe ✔ validate ✔ version-gate ✔ mdlint ✔ | `/unlock` reads `BWS_ACCESS_TOKEN`; D53 corrected |

**`/unlock` had never worked, and #217's filed fix would not have made it work.** `SKILL.md:50` read `TOKEN="$TROY"` and `:53` hard-stopped when empty; `TROY` is set by nothing on this machine, while `BWS_ACCESS_TOKEN` is set and current. So the skill printed "TROY environment variable is not set" and never reached `bws`, on every invocation. Second in-repo blocker, unfiled: `allowed-tools` omitted `python3` (`Bash(python:*)` does **not** prefix-match `python3`), `mktemp`, `chmod`, `rm`, `source`, `test`, and did not cover `:92`'s `BWS_ACCESS_TOKEN="$TOKEN" bws …` form, which is not `bws`-prefixed. Fixing the variable alone would have moved the failure one step later.

**A Phase-5 DoD check of my own was wrong, in the way this notebook keeps recording.** It read `grep -rn 'bws secret list' plugins/ | grep -v '>/dev/null' | grep -v 'TOKEN'`, and passed only because the functional calls happened to carry `BWS_ACCESS_TOKEN=` on the same line. 5.1 had to split that inline `VAR=… bws` form into `export`/`unset` — precisely because `Bash(bws:*)` never covered it — and the check immediately began reporting three legitimate lines. **The check was coupled to an incidental token on the line rather than to the property it meant to assert.** Replaced with a derived form: a leak is a `bws secret list` whose output reaches stdout, neither captured (`$(...)` or a PowerShell `$x = bws`) nor discarded, excluding markdown table rows. Negative-tested both ways — passes on the real tree, fires on a planted bare probe. First attempt at the replacement *also* false-positived on `new-project:62`, whose capture has an env-var prefix between `$(` and `bws`; that is two wrong checks before a right one, on a four-line grep.

**Only one genuine leak existed.** `references/api-key-setup.md:49` used a bare `bws secret list` as an auth diagnostic, which prints every secret's plaintext value. The three remaining occurrences are functional (two capture to a variable) or prose (an error-table row) and were correctly left alone — the issue's implied sweep of six out-of-repo scripts was moot: those use legacy `bw`/`BW_SESSION` and contain zero `bws` references.

**D53 corrected in place, not deleted** (Rule 4). Its mechanism named `~/.bashrc:167`'s interactive-only guard; the operative line is `~/.bashrc:8`, an early `return` for non-interactive shells that makes the anti-staleness `eval` at `:165` — already #217's proposed fix, present since 2026-07-16 — unreachable. The E058 incident was real but transient and host-level, and does not reproduce.

**Live `/unlock` verification is owner-run, not agent-run.** The skill is `disable-model-invocation: true` by policy (D40, user-invoke-only) and CI carries zero secrets (ADR-0009/D32), so an agent self-invoking it to satisfy an acceptance criterion would violate the very posture the skill exists to enforce. 5.1 adds the #232 mitigation to the skill itself: before trusting a manual run, diff the installed cache copy against the repo copy and abort if they differ — content, never the version string.
| 6 | 6.1-6.2 | opus + sonnet (parallel) | (this) | inventory-sync ✔ lint-CI-form ✔ pytest 67 ✔ version-gate ✔ validate×3 ✔ | CLAUDE.md inventory now generated + gated; 6 deprecated sites cleared; slide-gen 1.4.0 |

**#206's inventory half had a controlled experiment sitting in the tree.** `scripts/update-readme.py:329` hard-coded `README.md` as its only target, so the CI gate at `validate.yml:318` was green-by-construction on `CLAUDE.md` drift. README carries the same facts, is regenerated, and is provably drift-free; `CLAUDE.md` is hand-edited and carried **8** defects — 5 missing skills, `build-cfa-deck` missing from slide-gen, and **two whole directories absent** (`plugins/personal-plugin/agents/`, ten files and one of the repo's largest surfaces, plus `plugins/slide-gen/references/`). Not a missing generator: a generator with under-scoped coverage.

**The negative test found two more fail-open holes than the one it was written for.** With the `<!-- BEGIN:inventory -->` anchors deleted, and again with a plugin missing its tree node, the extended `--check` regenerated nothing and exited **0** — a silently-unchecked file, which is the precise defect the gate exists to prevent, reproduced inside the gate being built to prevent it. Both now raise and exit 1. Independently re-verified here: skill removed → **2**, anchors deleted → **1**, restored → **0**, Command Patterns byte-identical to `main`, 29/29 skill names present.

**Scoping made the annotation-preservation risk structural rather than behavioural.** Regeneration is bounded twice — by the explicit anchors, and within them to `commands/`/`skills/`/`agents/` lines at indent 4 under a `plugins/<name>/` node — so the four hand-written annotations and the curated 13-of-23 Command Patterns table are *unreachable* by the generator rather than merely avoided by it. That is the difference between a rule the code obeys and a rule the code cannot break.

**`sg-optimize`'s `--output` default was left UNSTATED on purpose.** `:33` contradicted itself inside one parenthetical ("overwrites input with `_optimized` suffix" — two mutually exclusive behaviours). The truth is not knowable from this repo: slide-gen is an external-dependency plugin (ADR-0008/D23), the engine is private, and `grep -rl "_optimized" --include=*.py` returns zero hits. The fix documents the uncertainty and points at `sg optimize --help`. Asserting either candidate would have been fabrication, and a confident wrong answer is worse than a visible contradiction.

**The parallelization disagreement the STARTUP scan flagged was resolved on evidence.** The Phase 6 header said `Sequential` while both items declared `Depends On: None` and shared zero files. Ran them concurrently; no conflict, and each agent correctly refused to touch the other's files when it found them already dirty.
| 7 | 7.1-7.2 | sonnet ×2 (parallel) | (this) | frontmatter ✔ no-stray-menus ✔ injections ✔ version-gate ✔ headers≤12 ✔ | 4 consent gates converted; 3 generator templates + the ask-questions residual |

**The frontmatter hazard was navigated, and it is worth stating why it was a hazard.** `security-analysis/SKILL.md:5`'s `allowed-tools` is a plain scalar terminated by two spaces and a `#`, and that trailing comment *contains* `unscoped: ` — a colon-space, inert only because it sits after the hash. Inserting the new grant on the wrong side of the `#` would have silently dropped the entire frontmatter: `name`, `allowed-tools`, and the D40-class flags, with no crash and no visible symptom. Verified after the edit by `yaml.safe_load`-ing all three converted skills and asserting a full key set, not merely that `--strict` exited 0. D39's unscoped `Bash` and its inline justification survive untouched.

**A defect my own verification caught that the agent's did not: a 13-character `header`.** `AskUserQuestion` caps `header` at 12 chars, and `"Security Scan"` is 13 — it would have been rejected at runtime, turning a converted gate into a broken one. Swept all 40 `header` values in `plugins/` afterwards; that was the only one. Worth noting the symmetry with the previous phase: there the agent caught my error (the "19 self-test cases" miscount), here I caught the agent's. Neither pass alone was sufficient.

**And a defect my verification INVENTED.** My first JSON check reported three "PARSE FAIL — Extra data" results; the regex was capturing across fence boundaries, so it was feeding a JSON object plus following prose to `json.loads`. The agent's per-block checks were correct and mine was wrong. A verification script can produce false positives as readily as false negatives, and a false positive costs the same investigation time as a real defect — the only defence is to look at the actual bytes before believing either.

**`security-analysis`'s scan-mode payload survived the conversion.** Its yes-branch sets `--dependencies-only` as the default and documents `--quick`/full overrides — behaviour, not decoration. A naive two-option yes/no conversion would have silently deleted a capability, which is #202's exact failure mode and the reason D64 named the payload explicitly.

**`interactive.md` was deliberately NOT converted** (plan v12 item 7.5 task 4): its one-at-a-time `[A]/[B]/[C]/[D]/[S]` rule is an intentional interview contract, not residue. The two remaining `[D] Custom` mentions elsewhere are prose stating the rule that they must not be re-added — a grep for the rendered artifact necessarily also matches the rule forbidding it.
| 8 | 8.1-8.3 | sonnet ×2 + orchestrator | (this) | no-stamps ✔ no-phantom-cmd ✔ validate×3 ✔ lint-CI ✔ evals ✔ injections ✔ inventory ✔ 492 tests ✔ | 12 stamps + both generators; D65 reconciled; task-sync 480→492 tests |

**8.1 required judgement, not a sweep, and the two things it did NOT delete are the evidence.** `create-wiki`'s `Last lint: never` was **kept** because `wiki/SKILL.md:236` genuinely rewrites that field on every `/wiki lint` run — it is a *backed* mechanism, and deleting it would have removed working information to satisfy a grep. `build-cfa-deck:217` ("verified only on 1.0.2") was **left alone**: it carries no date and is a genuine version caveat, not an unbacked freshness claim. At three further sites the caveat was kept and only the date dropped — "verify with provider if errors occur" is honest; "default as of 2026-03-31" asserts a check nobody performed. The distinction the item turns on is *backed vs unbacked*, not *dated vs undated*.

**The phantom was worse than the stamp.** `research-models.md:41`'s Resolution Order named `check-models`, a command deliberately deleted in plan v6 whose reference was never cleaned up, and `:70-80` documented that phantom's output. A reader following the file's own instructions would have run a command that does not exist. Both gone.

**Both generators stopped.** `explain-project/SKILL.md:392` *prescribed* "**Last verified:** date when claims were last checked" into every document it produces, and `create-wiki:271` templated a stamp. Fixing the 12 instances and leaving these two would have minted new ones indefinitely — E060 finding 3 / D56, and the fourth time this plan has had to fix a mould alongside its castings (the others: `adr-template.md` in Phase 4, the three `references/` templates in Phase 7, `update-readme.py` in Phase 6).

**8.3 closed the gap the coverage report had been naming all along.** `__main__.py` sat at 95% with Missing `281-283, 288` — and 288 *is* the `_split_flat_decisions` call, so the function written to fix `bug_001` had never executed through the CLI. 12 new CLI-level tests drive `run_sync(--apply)` with a real `--decisions` file across all five accepted shapes plus a flat-empty guard, a degenerate byte-identical-sections case, and five out-of-set values. Shapes are parametrized off the real `_DECISION_SECTIONS` constant, never a copy — the exact drift that produced #208/E056. **Negative-tested**: reverting `_split_flat_decisions` to return its input turns the new flat case red with `ValueError: orphan decision for unknown task 't-c'`. 480 → **492** tests, coverage 96.30% → **96.54%**, `281-283` and `288` now covered.
...
**Status:** COMPLETE — merged `776e140`; 21 → 9 open issues; all three caches content-verified
...
**Merge landed:** squash commit `776e140` on `main`, 22/22 checks green on the final head (the notebook-entry commit re-triggered the full suite). PR #236 MERGED.

**Hypothesis 2 was WRONG, and the failure is instructive.** Only **2** of the 12 issues auto-closed (#206, #210), not 12. GitHub's closing-keyword parser consumed only the leading entries of `Closes #206, #210, #217, …` — the keyword must precede *each* number, and a comma-separated list after a single keyword does not close the tail. Open count went 21 → **19**, not 21 → 9.

The remaining 10 were then verified individually against merged `main` before being closed by hand — not closed on the strength of the plan's own status fields. Spot evidence: `/unlock` now resolves `BWS_ACCESS_TOKEN` with `TROY` retained as a documented deprecated fallback (#217); `Last Verified` and the phantom `check-models` are both at zero occurrences in `research-models.md` (#218); `check_version_bump.py` is referenced once in `validate.yml` and three times in `scripts/pre-commit` (#226); `run_sync` is driven through `sync --apply --decisions` at `test_plan_apply.py:766` (#224); zero `L0-L4` occurrences in `ultra-plan/SKILL.md` (#231). Final count: **9 open**, exactly the predicted set.

**Worth carrying forward:** a `Closes` list in a PR body is not a closing mechanism, it is *one* closing keyword plus prose. Write `Closes #A, closes #B, closes #C`, or verify the close set after every merge. This one silently left 10 issues open on a PR whose body named them.

**Hypothesis 3 held, and the version string was again not the evidence.** The cache is **version-partitioned** — `~/.claude/plugins/cache/troys-plugins/<plugin>/<version>/` — which my first diff missed by comparing one level too high. Against the correct path, `personal-plugin` 11.7.0 is byte-identical to the repo: **239 tracked files, 0 missing, 0 differing, 0 extra**, `gitCommitSha` `776e140`. `slide-gen` was found stale at **1.3.0** (it bumped to 1.4.0 in Phase 6 and the resume brief named only personal-plugin); updated, now 14/14 identical.

**`bpmn-plugin` reproduced #226's failure mode live, in the session that shipped its fix.** `claude plugin update bpmn-plugin@troys-plugins` reported "already at the latest version (4.4.0)" — a version-string comparison — while the installed tree at that same 4.4.0 **differs in content**: its `CHANGELOG.md` is 194 lines to the repo's 214, missing the `4.4.0` entry itself plus `2.1.0`/`2.0.0`. The installed copy sits at sha `49face42`, which predates Phase 1's CHANGELOG backfill.

This is not a defect in the new gate — it is the gate's **exemption boundary made visible**. `EXEMPT_EXACT` is `CHANGELOG.md`, `LICENSE`, `README.md`; `EXEMPT_GLOBS` is `tools/*/tests/**`, `examples/**`. Every one of those paths may legitimately change without a bump (Phase 1 depended on exactly that), and because propagation is keyed on the version string, **exempt-from-bump also silently means never-reaches-an-installed-user**. Four of the five classes are harmless there (changelog/licence/readme are not read by the loader; tests do not ship as behaviour), but `examples/**` is user-facing content that bpmn-plugin actually has. Recording the consequence rather than changing the exemption set: the exemptions are correct for *release integrity*, and this is a *distribution* property nobody had stated.

**Direct evidence for #232, gathered in passing.** The cache retains **every** historical version directory — personal-plugin has 14 (`10.0.0` … `11.7.0`, including the `11.3.0` that #232 reports the loader serving), bpmn-plugin 3, slide-gen 3. `installed_plugins.json` names exactly one `installPath` per plugin. So the precondition for #232 is structural and still present: several complete, loadable plugin trees sit on disk under one marketplace, and the manifest's authority over which one the loader picks is exactly the thing #232 observed failing. That narrows #232 from "stale cache" to "resolution disagreement between the manifest and the loader across a retained version set."

**Status: COMPLETE.**

### Entry 065 — Fix #235: give the implement-plan state file a plan identity [command] [debug] [decision]

**Date:** 2026-07-30
**Environment:** Linux VM, branch `fix/plan-identity-fingerprint` off `main` `776e140`. personal-plugin 11.7.0. Release-integrity gate live, so any edit under `plugins/personal-plugin/` requires a bump plus a CHANGELOG entry.
**Status:** COMPLETE — `plan_identity` added, FINALIZATION ordering fixed, 5/5 fingerprint tests pass; personal-plugin 11.8.0

**Objective:** Close #235 — `/implement-plan`'s state file identifies its plan by **path**, so a completed run's state is inherited by a different plan written to the same path, and the run reports success having done nothing.

**Investigation, before any edit.**

*The fingerprint cannot be over the plan text, and the archived v12 plan proves it.* `/implement-plan` **decorates item headings on completion** — `#### 1.1 ADR-0011: … ✅ Completed 2026-07-28` — and 42 of 42 headings in `docs/archive/IMPLEMENTATION_PLAN-v12.md` carry that suffix. It also appends a `**Completed:**` header line at finalization. So a whole-file hash, or a hash over headings, would differ from itself after the first item and **break every legitimate resume** — converting a data-loss bug into a resume-never-works bug. Measured as invariant across that same completed run: `**Generated:**` (still `2026-07-29`, untouched), `**Total Phases:**`, the eight `## Phase N: …` heading lines (undecorated), and the `N.M` item *numbers*. Those four are the fingerprint; `**Completed:**` is deliberately excluded.

*The second failure is not the one E063 recorded, and the correction matters.* E063 states FINALIZATION's `rm -f .implement-plan-state.json` "runs only on the ALL_COMPLETE path and never executed when E061 ended at PR creation." The `rm` is **Final Step 0** (`implement-plan.md:380-386`) — it runs *before* documentation polish and *before* PR creation, so that ordering claim is inaccurate. The real defect is a contradiction inside the spec: Final Step 0 deletes the state file; **Final Step 3** (`:413-415`) then outputs the COMPLETION REPORT, whose generation rule 1 (`:372`) is *"read the state file"* and whose rule 5 (`:376`) says a missing state file means **"No state file found. No work items were completed."** A fully successful run must therefore either emit a flatly false report or decline to delete the file. An orchestrator that needs the file four steps later will not delete it — which is a sufficient mechanism for E061's file surviving, and is a defect in its own right independent of #235.

**Hypothesis:** Adding a `plan_identity` object to the state file, written at STARTUP Step 2 and verified at STARTUP Step 0 *before* the IN_PROGRESS check, makes the #235 scenario detectable rather than silent. Measurable success criteria: (a) the four fingerprint fields are each demonstrated invariant against the completed v12 plan, not asserted — a mismatch check that fires on a legitimate resume is worse than no check at all, and that is the specific way this fix can fail; (b) the mismatch path is a prompt, not a hard stop, and it defaults to discarding the stale state; (c) the identity check precedes the IN_PROGRESS check, because `in_progress` entries belonging to a different plan are meaningless; (d) FINALIZATION's ordering contradiction is resolved by moving the deletion after the COMPLETION REPORT, so the success path both reports truthfully and reliably removes the file; (e) `references/implement-plan-state-schema.md` moves in the same commit — it is the file `implement-plan.md:90` tells the orchestrator to read for the exact shape, so leaving it stale reproduces #232's "current tool, stale contract" shape inside this very fix.

**On the prompt form:** prose, not `AskUserQuestion`. D64 scopes the `AskUserQuestion` convergence to gates that are *both* model-invocable and consent to a write; `implement-plan` is a command, so it is user-invoked and the self-initiative hazard does not apply. The file's existing Step 0 retry/skip/complete prompt is already prose, so prose is also the local convention.

**Rollback Plan:** All work is on `fix/plan-identity-fingerprint`; `main` is untouched until a PR merges. Whole-change abort: `git checkout main && git branch -D fix/plan-identity-fingerprint`. Every file touched is git-tracked markdown, so `git checkout -- <file>` reverts any single edit. The version bump and CHANGELOG entry are part of the same commit and revert with it. No state, cache, or remote artifact is modified — this change edits documentation-as-behavior only, and nothing executes until a future `/implement-plan` run reads it.

**Results.**

**The fingerprint was verified invariant before being trusted, and that test is the load-bearing one.** Reconstructed the pre-execution form of `docs/archive/IMPLEMENTATION_PLAN-v12.md` (strip the ` ✅ Completed` suffixes, drop the `**Completed:**` header, revert `**Status:**` to `PENDING`) and compared fingerprints across a genuine 42-item run: **identical**. Four further checks — v13 item enumeration, prefix-vs-suffix decoration equivalence, a version-like string in an item title, and the actual #235 scenario (v12 state against the v13 plan at the same path) — all pass. 5/5.

**The first derivation was wrong, and the plan file itself is what caught it.** `^#### [0-9]+\.[0-9]+` returned **17** items for plan v13, which has **19**. Two items were decorated as a *prefix* rather than the template's documented suffix — `#### ✅ Completed 2026-07-30 — 7.1 Convert the 4 model-invocable write-consent gates` — so the anchored regex skipped them silently. Not a hypothetical: a guard whose item set is quietly short by two would report a spurious mismatch against its own plan on the next resume. Replaced with `awk 'match($0, /[0-9]+\.[0-9]+/)'`, which takes the first `N.M` anywhere on the line and is therefore indifferent to decoration placement; verified it still resolves `#### 3.1 Bump personal-plugin to 11.7.0` to `3.1` and not `11.7`.

This is the "derive it, don't restate it" rule again, at one more remove: I derived the regex from what `plan-template.md` *says* the decoration looks like rather than from what the decorated files actually contain. The template's own claim was the external truth I restated. `references/plan-template.md` now states the append-only rule normatively — the mould, not just the casting.

**E063's account of the second failure is corrected.** It recorded FINALIZATION's `rm` as running "only on the ALL_COMPLETE path" and not having executed "when E061 ended at PR creation". The `rm` was **Final Step 0** — ahead of both docs polish and PR creation — so that sequence could not have happened as described. The actual defect is a contradiction in the spec: delete at Final Step 0, then read the deleted file at Final Step 3 to build the COMPLETION REPORT, whose documented fallback for a missing file asserts **"No work items were completed."** A successful run had to either lie or keep the file. Deletion is now Final Step 4, explicitly the command's last action, with the ordering constraint stated in both directions so neither half can be "tidied" away later.

**What Worked.** Three things, worth naming because they were cheap:
- Reconstructing a pre-execution plan by *inverting the documented mutations* gave a real before/after pair without needing a plan mid-flight. The test that mattered most (does this reject legitimate resumes?) was otherwise unrunnable.
- Testing against the largest completed plan in the repo, not a fixture. A fixture would have had tidy suffix decorations and would never have exposed the prefix form.
...
**Correction, made before the PR merged — `/reload-plugins` is the remedy, not a restart.**

The owner ran `/reload-plugins` immediately after the results above were written, which is a direct test of the characterization. It **re-resolved in place**: at 15:35:57 this same PID 193416 gained markers for `personal-plugin/11.7.0`, `bpmn-plugin/4.4.0` and `slide-gen/1.4.0` — all three now matching `installed_plugins.json`, in a process that had been serving 11.3.0 / 4.3.1 / 1.2.0 since the previous morning. So a full process restart is **not** required, and the remedy I had written into the CLAUDE.md rule, the CHANGELOG and the preflight's warning text was stronger than the evidence supported. Corrected in all three before merge. `claude plugin update`'s own "Restart to apply changes" is true but overstates the requirement inside a Claude Code session.

**The marker-accumulation mechanism is now confirmed rather than inferred.** The old markers were **not** removed — 11.3.0, 11.5.1, 4.3.1 and 1.2.0 all remain alongside the new ones. Markers are additive per resolution event and never cleaned up, which is exactly why the count tracks process age. That was the inference behind the dose-response table; it is now observed directly.

**And the residual mystery is better characterized, though still not solved.** The new timestamps make the start-up sequence legible: `bpmn-plugin/4.3.1`, `slide-gen/1.2.0` and `personal-plugin/11.3.0` all carry the **identical** timestamp `09:58:48.745`, while `personal-plugin/11.5.1` sits alone 3.6 seconds earlier at `09:58:45.097`. So there were **two distinct resolution passes** at start-up: a personal-plugin-only pass that found 11.5.1, and a later all-plugins pass that found 11.3.0 / 4.3.1 / 1.2.0 — and the bulk pass is the one whose result served. It resolved *every* plugin to an older version, consistently. That is not randomness and not a per-skill effect; it looks like a bulk pass reading a persisted or cached resolution set rather than the live manifest. Still not determinable from disk, and still the only piece worth an upstream report — but now with a sharper question than "why 11.3.0": *why does the bulk start-up pass disagree with the manifest, and why does it win?*

### Entry 067 — Archive plan v13 and scope plan v14 [plan] [cleanup] [decision]

**Date:** 2026-07-30
**Environment:** Linux VM, branch `plan/v14-remaining-backlog` off `main` `713b406`. personal-plugin 11.9.0 · bpmn-plugin 4.4.0 · slide-gen 1.4.0 · marketplace 3.3.0. Session now serving 11.9.0 (verified by content after `/reload-plugins`). Backlog **8 open**, no P0 or P1.

**Objective:** Clear the D46 precondition by archiving the completed `IMPLEMENTATION_PLAN.md` to `docs/archive/IMPLEMENTATION_PLAN-v13.md`, and record the scope decision for the next plan.

**Scope decision (owner, this session): five issues — #200, #199, #238, #216, #198 — with the calibration halves MEASURED rather than asserted.** Deferred: #169/#170/#171.

Rationale for the deferral, since D58 already classed these as "own plan each" and this reaffirms it with fresh grounds: all three are v2 *features*, not defects, and each says `Priority: low / someday` in its own body. **#170 was already weighed and rejected during brainstorming (E047)** — filing a scope-out as an issue does not reopen the decision that scoped it out. **#169 got weaker after filing**: D65 made `tasks.json` local-only, so there is no cross-machine sync of the *public* list either, which is broader than the gap #169 describes. **#171 is the valuable one and is precisely why it should not ride along** — it is additive integration into `/ultra-plan` *and* `/implement-plan`, the two largest commands in the repo, and would dominate a plan whose other four items are markdown staleness fixes. Folding features into a correctness plan is what D54 rejected.

**The load-bearing distinction in this scope is factual staleness vs calibration judgement**, and it is why #198 changes shape rather than being dropped or taken as filed:

- **Factual, verified live against `main` `713b406` before planning:** `analyze-transcript` still carries the 50K/20K thresholds (3 sites), `assess-document` the ~100K, `plan-improvements` the 40%/60% reserve, `summarize-feedback` the Context Size Guardrail — and `consolidate-documents` already carries the context-*relative* exemplar to copy. `ultra-plan/SKILL.md:9` still opens with the bare `ultrathink` keyword, ~~a no-op on the current model family~~ — **corrected:** this keyword is LIVE, not a no-op; see plan v14 Phase 2 and Entry 067 below — and the skill still has **no `effort:` field at all**. `ultrathink` also survives in `commands/plan-improvements.md` and `references/templates/planning.md`.
- **Calibration, with no evidence the dial is currently wrong:** #198's "move the sonnet/opus boundary up", #199's `effort: max` → `high` A/B on the two planners, and #199's high→medium downgrades. The 13/14 `description-triggers` eval gives no signal either way, which is exactly the basis on which D54/D58 deferred them.

**Correction to #199 as filed, found while verifying:** its own body proposes riding along the `ultra-plan` phase-numbering repair, which **#231 already completed** in plan v13 Phase 4. That half is done and must not be re-planned. Separately its count is understated in one direction and overstated in another: **30 of 63** components carry no `effort:` field (the issue says ~15), but the issue only *names* ~15 where it argues the field matters — so 30 is the population, not the work item. Do not plan against 30.

**Hypothesis:** archiving is a pure move and leaves the tree otherwise unchanged. Measurable success criteria: (a) `docs/archive/IMPLEMENTATION_PLAN-v13.md` is byte-identical to the archived `IMPLEMENTATION_PLAN.md`; (b) it is actually tracked by git — `docs/archive/` matches the global `archive/` ignore, so a plain `git add` silently skips it and `git add -f` is required; (c) the working `IMPLEMENTATION_PLAN.md` is removed so no completed plan sits at the default path, which is the #235 hazard (a state file naming a plan by path) with the plan side of the pair still in place.

**Rollback Plan:** `git checkout main && git branch -D plan/v14-remaining-backlog` discards everything; `main` is untouched. The archive is a `git mv`-equivalent within one commit, so `git revert` restores the plan to its original path intact. Nothing outside the repo is touched and no issue state changes.

**Results.** Archived. `git status` staged it as a **rename** (`R IMPLEMENTATION_PLAN.md -> docs/archive/IMPLEMENTATION_PLAN-v13.md`), which is the confirmation that criterion (b) held — `docs/archive/` matches the global `archive/` ignore, so this only tracks because of `git add -f`; a plain `git add -A` would have staged the deletion and silently dropped the archive, leaving the plan destroyed rather than moved. Criterion (a) verified by `cmp` below the six-line banner: byte-identical. Criterion (c) held — no plan file remains at the default path, which also removes the *plan* side of the #235 pair (a stale state file plus a completed plan sitting at the path a new plan will take).

The banner carries the Rule 4 bidirectional pointer: v13's outcome, its merge SHA, the 12 issues it closed, the versions it released, and forward references to E063 (execution) and E064 (merge + cache verification), plus a pointer to its successor.

**Results — plan v14 generated.** `IMPLEMENTATION_PLAN.md`, 8 phases / 25 items (max 5 per phase, under the 6 cap), 8 runnable Definition-of-Done blocks, markdownlint clean. `/ultra-plan` Phase 1 dispatched **6 parallel Explore agents** clustered by shared code path. **Every cluster returned corrections**, and the aggregate rate again tracks E060's 22-of-24 baseline.

**Three findings outrank everything that was filed, and two came from reading the shipped harness binary rather than reasoning from prose.**

**The `ultrathink` keyword is LIVE.** Recovered from Claude Code 2.1.220: a case-insensitive, word-bounded matcher applied to the *expanded body* of every command and skill at load; the exemption guard covers only MCP and memory-store sources, not plugins. **Three in-repo surfaces assert it is a no-op** — #199's body, `LAB_NOTEBOOK.md:1385`, and the E052 audit. A **second live site** nobody had found: `commands/plan-improvements.md:34` fires because the matcher is case-insensitive and parentheses are word boundaries, so that command carries **two stacking escalations** (`effort: max` *and* an injection). `references/templates/planning.md:66` is the mould. This session's own transcript confirmed the mechanism twice more — the system-reminder fired on the skill load, then again on an agent report that merely *contained* the token, which is the E061 name-don't-render hazard in a new form and is now a constraint on how the fix is written.

**An absent `effort:` is exactly `high`.** Proven through the resolver, the model catalog, and the final coercion. So **all three of #199's `high` recommendations are no-ops**, its priority ordering is inverted (only the `low`/`medium` items move anything, and they are *downgrades* the issue never labels as such), and its headline is backwards: `ultra-plan` is the **most**-escalated unconfigured component, not the least. Population is 31, not 30 or "~15"; `sg-full-workflow` is assigned two conflicting values by the issue; `develop-image-prompt`'s stated rationale argues against its own recommendation.

**SKILL.md bodies are not always-loaded.** The harness's skill-doctor legend: *"full SKILL.md loads only when it runs."* Only the description is per-turn. #238's cost model — and the audit finding it inherited — is wrong, and `LAB_NOTEBOOK.md:166` (E032) already recorded the correct version as *"a house rule, not a CI gate"*. The 500 figure is an authoring best-practice this repo mislabelled "the official 500-line budget." **The gate survives on an argument the issue never makes:** both over-budget files crossed in **one commit** (`1382a8a`), in a PR whose own plan verified a line budget for a *different* file.

**Corrections I made to the investigators, applying the repo's own doctrine to its own agents.** The #200 agent reasoned from "the practical output ceiling is 64K" — the ceiling is **128K**; 64K is a recommended *floor* at high effort. Its conclusion (do not rescale the output-reserve column) survives; its stated reason was inverted. The #199 agent's claim that writing `effort: high` *removes* the user's session override rests on a precedence chain its own quoted code appears to contradict; the plan asserts only the no-op claim. Pricing was verified against the sanctioned `claude-api` source rather than taken: the tier premium is **2.5× today**, reaching the ~1.7× #198 assumes only after 2026-08-31 — so no default is flipped in this plan.

**Scope decisions taken by the owner before generation:** #216 ships **transport only**, ladder unchanged (current guidance for the default model says start mid-range and sweep *down*, and calls the effort dial an unreliable lever on visible output length — which is what a research leg produces); #238 ships as a **reframed authoring gate**; #198 ships **Rule 17 only**, no replay experiment.

**#216 is better justified than filed and was reframed accordingly.** The shipped comprehensive tier already runs at **double** the documented non-streaming output ceiling, so this is a latent correctness fix, not only an enhancement. The investigation also found a **latent defect nobody filed**: the skill's `allowed-tools` omits the interpreter grant that its own shipped fast-fail check already invokes — never caught because the prior live verification ran through a standalone probe rather than a real dispatch, and that probe was never committed.

**Four DoD rows were negative-tested before the plan shipped, and three were rewritten.** The keyword sweep correctly exits non-zero today and names exactly the three files the investigation found. The model-generation check fires on a planted violation. The body-length property lists exactly the two over-budget files. But the Phase 3 frontmatter row was **vacuous** — it inspected only changed files, of which there were none, so it passed while proving nothing; replaced with a whole-corpus form that exits 1 on a planted colon-space `description`. A Phase 2 row read "then inspect," which is not a gate at all; replaced with three assertions on strings that must survive. A Phase 6 row used a fragile compound; replaced. Last plan shipped four wrong DoD rows because they were written from what the author expected the command to match — this pass caught three of its own before generation rather than during execution.

**Status: COMPLETE.**

--- New session: 2026-07-31 — execute plan v14 via `/implement-plan`: 8 phases / 25 items off `main` `ac11a4e`. ---

### Entry 068 — Executing plan v14 (8 phases / 25 items) [plan] [build] [ci] [decision]

**Environment:** `main` @ `ac11a4e`, 0 divergence from `origin/main` (fetched). marketplace 3.3.0 · personal-plugin 11.9.0 · bpmn-plugin 4.4.0 · slide-gen 1.4.0. No `.implement-plan-state.json` present; v13 archived, so the D46 two-plans-at-one-path hazard (#235) is clear. Backlog 8 open, no P0/P1.

**Objective:** Execute `IMPLEMENTATION_PLAN.md` (plan v14) end to end — 25 items closing five issues (#200, #199, #238, #216, #198) — one branch and PR per phase, landing three doctrine corrections, one new bundled Python tool, one new CI gate, and a backward-compatible `visual-explainer` config change.

**Hypothesis:** All 8 phases land with every runnable Definition-of-Done block green, required check names unchanged, and the new budget gate green on arrival. Measurable success criteria:

- 25 of 25 items COMPLETE; zero items silently skipped (the #235 class).
- Every phase touching `plugins/` carries a version bump **and** a CHANGELOG entry — `python3 scripts/check_version_bump.py --base main` exits 0 at each phase.
- Case-insensitive sweep of `plugins/` for the keyword returns zero, and the sweep is negative-tested before it is trusted.
- `scripts/check_skill_budget.py` exits 0 against the tree *before* it is wired into `validate.yml`.
- Every accumulator guard in Phase 6 has a passing mutation test — deleting the branch flips the row from caught to passing.
- The 16 required status-check names on `main` are unchanged (the D55 deadlock hazard).

**Two decisions taken by the owner before execution, both recorded below as D69/D70 rather than left to the implementer:**

1. **Item 2.1 — `/ultra-plan` gets `effort: xhigh`.** The keyword deletion removes a real escalation; `xhigh` replaces an implicit, unsanctioned mechanism with an explicit, sanctioned one. `effort: high` is never written — it is a no-op.
2. **Phase 6 runs unattended.** Its DoD already requires fixture-first ordering and a mutation test per guard, which is the safety net the Critical risk needs.

**Rollback Plan:** Every phase is its own branch and PR off `main`, squash-merged. Per-phase rollback is `git revert <squash-sha>` on `main`; the pre-execution state is `ac11a4e` and is pushed, so `git reset --hard ac11a4e` restores the tree wholesale if the plan is abandoned. No system state outside the repo is modified: no plugin installs, no cache pruning (the E066 `.in_use` refcount finding forbids it), no external service configuration. Three items carry rollback notes beyond the git default:

- **Phase 6.1** creates a new tool directory — rollback is deleting `plugins/personal-plugin/tools/research-sse/` plus its CI wiring; nothing depends on it until 6.2 lands.
- **Phase 7.3** wires a new **step** into the existing `plugin-validate` job, never a new job (D28), so no branch-protection coordination is required and rollback cannot orphan a required check.
- **Phase 5.2** is backward-compatible by construction — an unset loop-tier override must be behaviourally indistinguishable from today, which is the acceptance criterion, so rollback is the null change.

**Known hazards carried into execution:**

- Phase 2's edits necessarily contain the literal keyword as deletions. Explanatory prose containing it must **not** enter any file under `plugins/` — it fires there. Explanations go in commit messages and this notebook (repo-root prose is outside the loader boundary; E067 already renders the token here safely).
- `commands/plan-improvements.md` is touched by three issues at four line regions and is serialized Phase 2 → Phase 4; each item enumerates the regions it must not touch.
- A comma-separated `Closes #A, #B, #C` closes only its leading entries (E064). The keyword goes before each number, and the close set is verified by listing after every merge.
- DoD rows are gates: each is derived from what CI actually runs, and any new one is negative-tested against deliberately-bad input before it is trusted (E063).

**Correction to the Rollback Plan as written above:** execution used a **single branch** (`feature/plan-v14`) with one commit per phase and one PR, not the "branch and PR per phase" the plan states. `/implement-plan` finalizes with a single PR, and one version bump covers the whole release (five bumps for one release would contradict D45). Rollback is unaffected -- revert the squash, or `git reset --hard ac11a4e`.

**Results -- 25 of 25 items COMPLETE, 7 commits, every phase DoD green.** `1080717` (P1) - `86dddee` (P2) - `fbeea83` (P3) - `bacbd10` (P4) - `6b7e951` (P5) - `e093488` (P6) - `0fa09af` (P7). personal-plugin **11.9.0 -> 11.10.0**, slide-gen **1.4.0 -> 1.5.0**, bpmn-plugin **unchanged and deliberately not bumped** (D45 -- never an empty coordinated bump).

**The dominant finding is that FIVE Definition-of-Done rows were wrong, all the same shape**, in a plan whose own preamble records four rows caught and rewritten *before* it shipped. The failure mode is not eliminated by having been named; it recurred at a lower rate.

1. **The release-gate row passes vacuously if run before the phase commit.** `check_version_bump.py --base main` diffs *committed* refs, so with Phase 2's work staged-but-uncommitted it re-checked Phase 1 and reported OK. A row that green-lights an unbumped release is the guard-that-cannot-fail class.
2. **Phase 4's absolute-table row FAILED against a correct implementation.** `^\| (Small\|Medium\|Large\|Very Large) \(` matches two tables -- the absolute budget table the item deletes and a wall-clock duration table that legitimately survives. Corrected to `! grep -q 'Output Reserve'`, the column that actually made the table absolute.
3. **Item 4.4's acceptance criteria contradicted each other.** AC4 required `:95` byte-identical; AC3 required the interaction payload to offer no option the body describes. After batching was removed, `:95` still offered "process in batches" -- an unbound instruction, the same defect class 4.2 was fixing two files over.
4. **Phase 7's `! grep -q 'commands'` row fails for a correct implementation** -- the docstring must name `commands/` to explain the exclusion, and a self-test case is literally named `commands-file-not-evaluated`.
5. **Phase 7's inline body-measurement idiom over-counts every body by exactly one.** `.split('---',2)[2]` begins with the newline terminating the closing `---`, so `splitlines()` yields a leading empty element (491/489/440 by the idiom vs 490/488/439 by the gate). Green either way today, but at the boundary it would fail a file the gate passes -- **and it corrupted a headroom figure the orchestrator itself reported** before the gate existed to contradict it.

**A concurrency defect that two green verifications did not catch (Phase 3).** Parallel items 3.1 and 3.2 both reported PASS on `sg-full-workflow` while asserting contradictory things about it: 3.1 verified it byte-identical to `main` (correct -- it was out of 3.1's scope), 3.2 verified it at `effort: medium`. On disk it had **neither**; 3.2's write was lost to 3.1's concurrent untouched-check. No DoD row would have caught it -- "no `+effort: high` additions" and "exclusions intact" both pass on an item silently not applied. Found only by enumerating the actual `effort` value of every changed file. **Structural rule adopted for the rest of the run: a file that is in-scope for one parallel item and explicitly out-of-scope for another must not be split across concurrent agents.**

**The Critical risk closed, and closed by test rather than by prose.** Under streaming the terminal reason moves from a top-level `stop_reason` field into a `message_delta` event, so a faithful-looking port keeps the old lookup and never fires. Verified directly: the naive lookup returns `None` on both refusal fixtures, and `refusal_with_category` carries 18 bytes of partial refusal text that would have *become* the report. A helper now performs that naive lookup against every refusal fixture and asserts `None`; all 12 accumulator guards are mutation-tested **fail-first** (base pass / mutant FAIL / restored pass), which was D70's stated residual. Two techniques worth keeping: mutants must run `--no-cov`, because a single test dropping below the coverage floor fails for coverage reasons and manufactures a false "caught" for every row; and `-w '%{http_code}'` under a pipe writes the status into the report body, so it had to become `--dump-header`.

**Grant gaps were far wider than filed.** #216 named the missing `python3` grant. The audit found `grep`, `sed`, `tail`, `tr`, `cut` also ungranted (OpenAI/Gemini legs) and `find` once 6.2 adopted the house path-resolution idiom -- seven binaries, all dispatch-time-only failures that every offline gate passes. One agent report claimed `Bash(command:*)` covers arbitrary commands; it grants the `command` builtin only. The identical gap exists in `summarize-feedback` and is filed as #243.

**What the run produced beyond the fixes:** a new bundled tool (`research-sse`, stdlib-only, 87 tests / 99%, wired into CI **non-required** first per the task-sync precedent), a new authoring gate (`check_skill_budget.py`, boundary negative-tested at 499/500/501 by construction, green on arrival), a committed live probe that *extracts* rather than restates the request shape, and four eval scenarios for a leg that previously had none.

**Six follow-ups filed:** #241 (effort A/B, confound now removed), #242 (tier-boundary replay), #243 (`summarize-feedback` find gap), #244 (root CHANGELOG unguarded), #245 (promote research-sse to required), #246 (`planning.md` mints a no-op `effort: high`).
