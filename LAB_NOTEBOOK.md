...
|---|--------|---------|-------------|
| — | **No open action items.** A22 closed (see C27); A23 closed in E062. The canonical backlog is the GitHub issues list (A2/A12), now **21 open** — 12 of them close on PR #236's merge, leaving 9: #198/#199/#200 (calibration, deferred), #206's context-economy half, #216, #232, #235, and #169/#170/#171 (v2 scope-outs). **Traps that recur:** `gh issue view <n>` silently resolves PR numbers; an artifact identified by a *label* rather than its *content* has now bitten three times (#226 version string, #232 cache path, #235 plan path) — verify content, never the identifier; `bws secret list` prints plaintext; any injection linter must **replay the pre-pass, not grep**; and a Definition-of-Done row must be **derived from the gate it mirrors**, not written from what you expect the command to match (four were wrong this session). | — | E043 |
